...
import { appendStreamingFormatHint, maybeAppendStreamingHint, getApiErrorMessage, MIME_MAP, normalizeBase64Image, pickActualParams } from '../imageApi/shared'
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    if (profile.streamImages && isEventStreamResponse(response)) {
      return await parseAgentStreamResponse(response, mime, controller.signal, signal, onTextDelta, onOutputItems, onImageToolStarted, onImagePartialImage, onImageToolCompleted, onImageToolFailed)
    }
